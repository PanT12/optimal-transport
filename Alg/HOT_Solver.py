...
class HOTSolver:
    def __init__(self,
        ...
        Args:
            original_C : original cost matrix
            b : LP problem, Ax=b
            c : LP problem, <c, x>
            m : 2D Grid rows
            n : 2D Grid cols
        ...
        """
        b = np.concatenate([np.zeros(m * n), a_seq, b_seq])
        ...
        # self.x = torch.ones(self.params, dtype=self.dtype, device=self.device) / n  # primal problem variable
        # self.x_bar = torch.ones(self.params, dtype=self.dtype, device=self.device) / n
    ...
    def set_initial_x(self, x0_vec, in_original_scale: bool = True):
        """
        将初始点写入 x, x_bar, x0。
        x0_vec: 形状=(self.params,) 的 torch/numpy 向量
        in_original_scale=True 表示传入的是“原始尺度”，函数会自动除以 b_scale 存到内部。
        """
        x0 = torch.as_tensor(x0_vec, dtype=self.dtype, device=self.device)
        if in_original_scale:
            x0 = x0 / self.b_scale
        if x0.numel() != self.params:
            raise ValueError(f"x0 length {x0.numel()} != params {self.params}")
        x0 = torch.clamp(x0, min=0)  # 保险起见
        self.x.copy_(x0)
        self.x_bar.copy_(x0)
        self.x0.copy_(x0)
    ...
    def optimize(self, time_max=np.inf, verbose=True):
        self.normalize()
        self.bc_scale = self.b_scale * self.c_scale * self.c_Max

        # initial point
        X = np.outer(self.a_seq, self.b_seq)
        f2 = X.flatten(order='F')
        x0_vec = torch.tensor(np.concatenate([self.a_seq, f2]))
        self.set_initial_x(x0_vec, in_original_scale=True)
        obj_past = self.c.dot(self.x_bar) * self.bc_scale
        self.history['cost'].append(obj_past.item())
        self.history['time'].append(0.0)
        self.history['kkt_err'].append(1.0)

        if self.obj_truth is not None:
            relative_error = abs(obj_past - self.obj_truth) / self.obj_truth
            absolute_error = abs(obj_past - self.obj_truth)
            self.history["rel_err"].append(relative_error.item())
            self.history["abs_err"].append(absolute_error.item())

        if verbose:
            print("Initial Checking...")
        self.KKT()
        self.display_info(verbose)
        if verbose:
        ...
        while self.flag and self.iter_count < self.max_iters:
            ...
            if self.iter_count % self.check_freq == 0:
                ...
                self.KKT()
                self.history["kkt_err"].append(self.kkt_res.item())
                if self.kkt_res <= self.tolerance or t_total > time_max:
                    self.flag = 0
                if self.adjust_sig:
                    self.adjust_sigma()
                ...
                cost = self.c.dot(self.x_bar) * self.bc_scale
                self.history["cost"].append(cost.item())
                self.history["time"].append(t_total)

                if self.obj_truth is not None:
                    relative_error = np.abs(cost - self.obj_truth) / self.obj_truth
                    absolute_error = np.abs(cost - self.obj_truth)
                    self.history["rel_err"].append(relative_error.item())
                    self.history["abs_err"].append(absolute_error.item())
                t_start = timeit.default_timer()

                # print("HOT primal feasiblity is ", self.log['pr_feaserr'])
                # Y = self.recover_transport_plan()
                # row_sum = Y.sum(dim=1)
                # col_sum = Y.sum(dim=0)
                # print("cost is ", self.prime_optimal.cpu().item())
                # print("our cost is ", cost.cpu().item())
        if self.logging:
            ...
            return self.recover_transport_plan()
    ...
    def recover_process(self):
        x = torch.mul(self.x, self.b_scale)
        x = torch.maximum(x, self.zero)
        to_mid = x[:self.m2n].reshape((self.n, self.m, self.m)).permute(2, 1, 0)
        to_end = x[self.m2n:].reshape((self.n, self.n, self.m)).permute(2, 1, 0)
