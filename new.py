...
class BISNsolver_EOT(optimal_transport):
    ...
    def accelerated_gradient(self, g, fixed_term, first_stage_tol, inner_iter, stepsize=1e-1):
        ...
        numerator = np.zeros_like(fixed_term)
        loss_list = []
        gnorm_list = []
        for i in range(inner_iter):
            ...
            if gnorm < first_stage_tol:
                # temp = (g[None, :] - self.C) / self.eta  # shape (n, m)
                #
                # logZ = logsumexp(temp, axis=1)  # shape (n,)
                #
                # alpha = self.eta * (np.log(self.a) - logZ)
                # print("dual variable 1 is ", alpha)
                # print("dual variable 2 is ", g)
                break
        P = numerator / row_sum
        newX = P * self.a[:, None]
    ...
    def optimize(self, tol, rho=None, first_stage_tol=1e-1):
        ...
        if not self.skip_first_stage:
            g, P, X, iter1 = self.accelerated_gradient(
                g, fixed_term, first_stage_tol, inner_iter=5000, stepsize=2*self.eta
            )
        else:
            X = P * self.a[:, None]
        # second-order method
        g, X, P, cg_iter, cg_time, iter2 = self.Newton_sort(
            g, X, P, log_fixed_term, fixed_term, tol, rho_=rho, inner_max_iter=500
        )
        ...
        print(f"iter = {iter2}, gnorm = {gnorm:4.1e}, time = {t_total:.2f}")
    ...
    def Sparsity_introduce(self, P, rho):
        ...
        if np.any(row_sum == 0.0):
            warnings.warn("Sparsity P: A row in P has all zero entries. It is better to increase regularization parameter eta.")
            zero_indices = np.where(row_sum == 0)[0]
...
if __name__ == "__main__":
    ...
    start_time = timeit.default_timer()
    X = solver.optimize(tol=1e-9, first_stage_tol=1e-3)
    print(X)
    end_time = timeit.default_timer()
